...
class TimeTableGenerator:
    def __init__(self, periods_per_day=7, days_per_week=6, fixed_subjects=None):
        self.days = days_per_week
        self.periods_per_day = periods_per_day
        self.total_periods = self.days * self.periods_per_day
        self.fixed_subjects = fixed_subjects or {
            "Training": 2,
            "Training Exam": 2,
            "Sports": 2,
            "Counseling": 1,
...
def assign_teachers(subjects, num_sections, fixed_subjects):
    teacher_map = {}
    teacher_pool = []
    teacher_id = 0
...
class TimetableBuilder:
    def __init__(self, sections, subjects, teacher_map, teacher_pool, fixed_subjects):
        ...
        self.allocations = {
            section: TimeTableGenerator(fixed_subjects=self.fixed_subjects).generate_for_class(subjects)
    ...
    def place_fixed_subjects(self, section_index, section):
        all_fixed = list(self.fixed_subjects.items())
        ...
        for subj, total_periods in all_fixed:
            ...
            if subj in {"Sports", "Library","Skills"}:
                # Place Sports and Library ONLY at the end of the day
                if total_periods == 2:
                ...
                # For other fixed subjects, place anywhere in the day
                if total_periods == 2:
                    for day in random.sample(range(self.days), self.days):
    ...
    def place_subjects(self):
        for section_index, section in enumerate(self.sections):
            ...
            for subject in self.subjects:
                if subject.name in self.fixed_subjects:  # Use passed fixed_subjects here too
                    continue
                ...
                for period in range(self.periods_per_day):
                    if self.timetable[section][day][period] is None:
                        sorted_subjects = sorted(
                            [s for s in self.subjects if s.type == 'theory' and s.name not in self.fixed_subjects],
